...
*   **EuiPageTemplate.Sidebar** extends [**EuiPageSidebar**](#/templates/page-template/examples#providing-a-sidebar)
*   **EuiPageTemplate.Header** extends [**EuiPageHeader**](#/layout/page-header)
*   **EuiPageTemplate.Section** extends [**EuiPageSection**](#/layout/page-components#page-sections)
*   **EuiPageTemplate.BottomBar** extends [**EuiBottomBar**](#/layout/bottom-bar)
*   **EuiPageTemplate.EmptyPrompt** extends [**EuiEmptyPrompt**](#/display/empty-prompt)
...
If you have a fixed position [headers](#/layout/header#fixed-header), the template will automatically account for the padding `offset` based on the total number of page headers. If you do not want the template to calculate this, you can pass `0` in as the manual `offset`.
...
This component will set its content to stick to the top of the browser window and scroll independently of the rest of the layout. If you have any fixed [**EuiHeaders**](#/display/page-header), these will be accounted for as well. You can turn this behavior off with `sticky={false}`.
...
Adding an [**EuiBottomBar**](#/layout/bottom-bar) can be tricky to use and account for any sidebars. **EuiPageTemplate** handles this nicely by supplying a **EuiPageTemplate.BottomBar** component for passing the contents of your bottom bar that extends **EuiBottomBar**.
...
When the content is in an empty/pre-setup state, we recommend then using an [**EuiEmptyPrompt**](#/display/empty-prompt) to direct users on next steps. Using **EuiPageTemplate.EmptyPrompt** will automatically center the prompt vertically and horizontally.
